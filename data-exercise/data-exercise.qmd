...
In this section we explore another dataset following similar steps above:
...
table(heights_clean$sex)


ggplot(heights_clean, aes(x = height)) +
...
ggplot(heights_clean, aes(x = sex, y = height)) +
geom_boxplot() +
labs(
title = "Height by sex",
x = "Sex",
...
# simple statistical model (linear model)


```{r}
fit_height <- lm(height ~ sex, data = heights_clean)
summary(fit_height)

```

The linear model shows that sex is a statistically significant predictor of height (p < 0.001). On average, males are estimated to be approximately 4.4 inches taller than females in this dataset. 
